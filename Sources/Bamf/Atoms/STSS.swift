...
extension Atom {
  /// Sync Sample Box (ISO 14496-12 §8.6.2)
  public class STSS: Atom {
    ...
    /// Sample numbers of sync (key) samples (1-based)
    public var sampleNumbers: [UInt32] {
      var result: [UInt32] = []
      ...
      for _ in 0..<count {
        ...
        let number: UInt32 = data[offset..<(offset + 4)].asInteger()
